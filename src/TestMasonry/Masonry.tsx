...
import { PostCard } from '../Blog/BlogComponents'
import { postLibrary } from '../BlogContent/PostLibrary'
const data = [
    { css: 'https://images.pexels.com/photos/416430/pexels-photo-416430.jpeg', height: 150, zIndex: 20 },
    ...
    { css: 'https://images.pexels.com/photos/327482/pexels-photo-327482.jpeg', height: 800 },
...
export function Masonry() {
    ...
    const ref = useRef()
    const width = useRefProp(ref, (el) => el.offsetWidth)
    ...
    */
    ...
    return (
        <div className='w-100 d-flex justify-content-center'>
            <div ref={ref} className='w-100 m-5'>
                <div className={styles.list} style={{ height: Math.max(...heights) }}>
                    {transitions((style, item) => (
                        <a.div style={style}>
                            ...
                            <CardWithHeight onHeightChange={(height)=>{
                                console.log("new height "+height)
                                set([...items.map((it)=>{
                                    it.height=height*2.2
                                    return it
                                })])
                            }} />
                        </a.div>
...
function CardWithHeight({onHeightChange}){

    const ref = useRef();
    const height = useRefProp(ref, (el)=>el.offsetHeight)
    ...
    return <PostCard cardRef={ref} postData={postLibrary.getLatestPosts()[0].getPostData()} />
}

function useElementProp(query: string, fn) {
    const [height, setHeight] = useState(undefined);

    useEffect(() => {
        const resizeObserver = new ResizeObserver(entries => {
            setHeight(fn(document.querySelector(query)));
        });
        var toObserve = document.querySelector(query)
